jQuery(function ($) {
    ...
    $(document).ready(function () {
        $(".item-details").on("click", function (e) {
            e.preventDefault();
            $(".item-details-modal").modal({ backdrop: "static", keyboard: false, focus: true });
            var url = $(this).attr("data-url");
            $("#modal-loader").show();
            $.ajax({
                type: "GET",
    ...
    $(document).ready(function () {
        $(".status-change").on("click", function (e) {
            ...
            var formData = { id: value[1], status: status };
            $.ajax({
                type: "POST",
                dataType: "json",
                data: { data: formData, _token: token, _method: "put" },
                headers: { "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content") },
                url: url,
    ...
    $(document).ready(function () {
        $(".product-status-change").on("click", function (e) {
            ...
            var formData = { id: value[1], status: status, change_for: value[2] };
            $.ajax({
                type: "POST",
                dataType: "json",
                data: { data: formData, _token: token, _method: "put" },
    ...
    $(document).on("click", "#banner_delete", function () {
        ...
        selector.hide();
        $(".delete-image").remove();
        $.ajax({ headers: { "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content") }, url: "payment-methode-banner", data: { title: "payment_method_banner" }, method: "post", success: function () {} });
    });
    $(".file-select").fileselect();
    $(".flags").select2({ templateResult: formatState, templateSelection: formatState });
    function formatState(opt) {
        ...
        if (!optimage) {
            ...
            var $opt = $('<span><img class="mb-1" src="' + optimage + '" /> ' + opt.text.toUpperCase() + "</span>");
            return $opt;
        }
    }
    $(function () {
    ...
    function getKey(length = 16, input) {
        ...
        for (var i = 0; i < length; i++) api_key += string.charAt(Math.floor(Math.random() * string.length));
        input.val(api_key);
    }
    ...
    $(document).ready(function () {
        ...
        $(".site-lang").on("change", function () {
            ...
            var formData = { lang: lang, url: url, title: title };
            $.ajax({
                type: "GET",
                dataType: "html",
                data: formData,
        ...
        $(".pos_invoice_lang").on("change", function () {
            ...
            var formData = { lang: lang, url: url, title: title, address: address, condition: condition, phone: phone, powered_by: powered_by };
            $.ajax({
                type: "GET",
                dataType: "html",
                data: formData,
    ...
    $(document).ready(function () {
        $(".account-active-status-change").on("click", function (e) {
            ...
            var formData = { id: value[1], status: status, is_active: value[2] };
            $.ajax({
                type: "POST",
                dataType: "json",
                data: { data: formData, _token: token, _method: "put" },
    ...
    $(".limit-2-custom-message .select2").select2({ multiple: "multiple", maximumSelectionLength: 2, language: { maximumSelected: (args) => "You can select only " + args.maximum } });
    $(".limit-3-custom-message .select2").select2({ multiple: "multiple", maximumSelectionLength: 3, language: { maximumSelected: (args) => "You can select only " + args.maximum } });
    $(".limit-4-custom-message .select2").select2({ multiple: "multiple", maximumSelectionLength: 4, language: { maximumSelected: (args) => "You can select only " + args.maximum } });
    ...
    function getStoreContent(type, div_area, content_number, for_content) {
        var url = $("#url").val();
        var formData = { type: type, content_count: content_number, for_content: for_content };
        $.ajax({
            type: "GET",
            dataType: "html",
            data: formData,
    ...
    $(document).ready(function () {
        $(".order-statistics").on("click", function (e) {
            ...
            $(".card-statistic-2").block({ message: "Processing...", css: { border: "none", padding: "15px", color: "#fff" } });
            $.ajax({
                type: "GET",
                dataType: "html",
                async: false,
    ...
    $(document).ready(function () {
        $(".top-products-state").on("click", function (e) {
            ...
            var report_ln = area.text();
            var path = area.closest(".dropdown-menu").attr("data-url");
            $(".card-statistic-6").block({ message: "Processing...", css: { border: "none", padding: "15px", color: "#fff" } });
            $.ajax({
                type: "GET",
                dataType: "html",
                async: false,
    ...
    $(document).ready(function () {
        ...
        $("#download_update").on("click", function () {
            var url = $(this).attr("data-url");
            $("#app").block({ message: "Processing...", css: { border: "none", padding: "15px", color: "#fff" } });
        ...
        $("#payment_type").on("change", function () {
            var opval = $(this).val();
            if (opval == "cash") {
                $("#modal_payment_type").removeClass("d-none");
            } else if (opval == "wallet") {
                $("#modal_payment_type").addClass("d-none");
